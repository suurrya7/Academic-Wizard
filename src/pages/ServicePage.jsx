                ...
                <div className="container px-6">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {service.countries.map((country, idx) => (
                            <section key={idx} id={`${service.slug}-${country.name.toLowerCase().replace(/\s+/g, '-')}`} className="glass-card p-6">
                                <div className="flex items-center gap-3 mb-4">
                                    <span className="text-2xl">{country.flag}</span>
                                    <h3 className="text-lg font-bold text-white font-heading">{service.title} in {country.name}</h3>
                                </div>
